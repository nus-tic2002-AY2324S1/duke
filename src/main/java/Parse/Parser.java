...
public class Parser {
    ...
    public int findIndex(String[] wordArray, String keyWord){

        assert wordArray != null : "Input array is null";
        ...
        switch(wordArray[0].toLowerCase()){
            ...
            case "view":
                if (wordArray.length == 2) {
                    ...
                    assert taskDate != null : "Parsing failed for input: " + wordArray[1];
                    if (taskDate == null){
            ...
            case "deadline":
                ...
                if (byIndex != -1 && byIndex > 1 && byIndex < wordArray.length - 1) {
                    ...
                    assert dateTime != null : "Parsing failed for input: " + dateTimePart;
                    if (dateTime != null){
            ...
            case "event":
                ...
                if (fromIndex != -1 && toIndex != -1 && fromIndex < toIndex && toIndex < wordArray.length - 1) {
                    ...
                    assert fromDateTime != null : "Parsing failed for input: " + fromDateTime;
                    assert toDateTime != null : "Parsing failed for input: " + toDateTime;
                    // Try to parse "from" and "to" date-time parts
                    if (fromDateTime == null || toDateTime == null) {

                        return new ParsedCommand("invalid", "Invalid date, time or date time format for 'event' command. It should be in the YYYY-MM-dd, HH:MM, or YYYY-MM-dd HH:mm format!");
                    } else {
                        return new ParsedCommand("event", taskDescription, fromDateTime, toDateTime);
                    }
                } else {
                    return new ParsedCommand("invalid", "Event command is missing the task description, 'from,' or 'to' components. Usage: event <task description> /from <date/time> /to <date/time>, where the format of <date> is YYYY-MM-DD, <time> is HH:MM, and <date time> is YYYY-MM-DD HH:MM");
                }
